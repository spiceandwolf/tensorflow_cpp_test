...
def load_and_encode_train_data(num_queries, num_materialized_samples):
    file_name_queries = "mscn/train"
    file_name_column_min_max_vals = "mscn/column_min_max_vals.csv"
...
def make_dataset(samples, predicates, joins, labels, max_num_joins, max_num_predicates):
    ...
    sample_tensors = tf.convert_to_tensor(sample_tensors)
    sample_masks = np.vstack(sample_masks)
    sample_masks = tf.convert_to_tensor(sample_masks)
    # print("\nsample_tensors: ")
    # print(sample_tensors.shape)
    # print("\nsample_masks: ")
    ...
    predicate_tensors = tf.convert_to_tensor(predicate_tensors, dtype=tf.float32)
    predicate_masks = np.vstack(predicate_masks)
    predicate_masks = tf.convert_to_tensor(predicate_masks, dtype=tf.float32)
    # print("\npredicate_tensors: ")
    # print(predicate_tensors.shape)
    # print("\nsample_tensors: ")
    # print(predicate_masks.shape)
    ...
    join_tensors = tf.convert_to_tensor(join_tensors, dtype=tf.float32)
    join_masks = np.vstack(join_masks)
    join_masks = tf.convert_to_tensor(join_masks, dtype=tf.float32)
    # print("\njoin_tensorss: ")
    # print(join_tensors.shape)
    # print("\njoin_masks: ")
    # print(join_masks.shape)

    target_tensor = tf.convert_to_tensor(labels, dtype=tf.float32)

    # return tf.data.Dataset.from_tensor_slices((sample_tensors, predicate_tensors, join_tensors, target_tensor, sample_masks,
    #                              predicate_masks, join_masks))
    return [sample_tensors, predicate_tensors, join_tensors, target_tensor, sample_masks,
                                 predicate_masks, join_masks]
